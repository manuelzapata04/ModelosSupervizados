...
def resultados(df,pipeline,tgt):
    df_re=df.copy()
    df_re["pred"]=pipeline.predict(df)
    df_re["proba"]=pipeline.predict_proba(df)[:,1]
    proba=[]
    n_fraude=[]
    fraude=[]
    ...
    resultado=pd.DataFrame({"Proba":proba,"Not Hazardous":n_fraude,"Hazardous":fraude})
    return resultado
...
def plot_estabilidad(df, pipe, tgt):
    prueba = resultados(df,pipe,tgt)
    prueba["false"]  = prueba["Not Hazardous"] /(prueba["Not Hazardous"] + prueba["Hazardous"])
    prueba["true"]  = prueba["Hazardous"] /(prueba["Not Hazardous"] + prueba["Hazardous"])
    prueba["false"] = prueba["false"].fillna(0)
    prueba["true"] = prueba["true"].fillna(0)
    prueba2 = prueba.melt(id_vars = "Proba", value_vars = ["false", "true"], var_name = "Hazardous")
    return px.bar(prueba2, x='Proba', y='value',
                  color='Hazardous', barmode   ='stack', width = 600, height = 350)
